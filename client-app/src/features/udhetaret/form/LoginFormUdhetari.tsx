...
export default observer (function LoginFormUdhetari() {
    const {udhetariStore} = useStore();
    return (
        <>
        <Formik 
            initialValues={{email:'', password: '', error: null}}
            onSubmit={(values, {setErrors}) => udhetariStore.loginUdhetari(values).catch(error => 
                setErrors({error:'Invalid email or password'}))}
        >
            {({handleSubmit, isSubmitting, errors}) =>(
                <Form className='ui form' onSubmit={handleSubmit} autoComplete='off' style={{height:"400px",padding:"15px"}}>
                    <h2 style={{textAlign:"center",color:"rgba(224, 130, 131, 1)",marginBottom:"30px",fontFamily:"arial"}}>Login User</h2>
                    <h4>Email:</h4>
